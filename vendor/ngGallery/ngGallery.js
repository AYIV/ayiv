(function () {
	...
	function ngGallery( $document, $timeout, $q, $templateCache ) {
		...
		$templateCache.put(template_url,
			...
			'    <img ng-src="{{ i.thumb }}" class="{{ thumbClass }}" ng-click="openGallery($index)" alt="Image {{ $index + 1 }}" title="{{ i.description }}"/>' +
			'  </div>' +
			'</div>' +
			...
			'  <span class="info-text"><input type="checkbox" ng-model="isDefault" />{{ index + 1 }}/{{ images.length }} - {{ description }}</span>' +
